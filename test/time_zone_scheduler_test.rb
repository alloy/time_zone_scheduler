...
describe TimeZoneScheduler do
  ...
  describe "#schedule_in_timeframe" do
    before do
      ...
      @from = ActiveSupport::TimeZone['Brazil/East'].parse('2015-10-25 10:00')
    ...
    it 'delivers in real-time where local time falls within the allowed timeframe' do
      ...
      TimeZoneScheduler.new('Brazil/East').schedule_in_timeframe(@from, @timeframe).must_equal(@real_time)
      # This is 2015-10-25 08:00 (America/New_York).
      TimeZoneScheduler.new('America/New_York').schedule_in_timeframe(@from, @timeframe).must_equal(@real_time)
      # This is 2015-10-25 15:00 (Europe/Amsterdam) with DST, but as during
      # this night DST ended, it is actually 2015-10-25 14:00.
      TimeZoneScheduler.new('Europe/Amsterdam').schedule_in_timeframe(@from, @timeframe).must_equal(@real_time)
    end

    it 'delays those that fall before the allowed timeframe till start same day' do
      # This is 2015-10-26 02:00 Pacific/Kiritimati.
      TimeZoneScheduler.new('Pacific/Kiritimati').schedule_in_timeframe(@from, @timeframe).must_equal(@real_time + 6.hours)
      # This is 2015-10-25 01:00 Pacific/Niue.
      TimeZoneScheduler.new('Pacific/Niue').schedule_in_timeframe(@from, @timeframe).must_equal(@real_time + 7.hours)
    end
